...
void strong_password_check(string& password)
{
    ...
    else if(regex_search(password, SpecialCases))
    {
        sumOfCases += 1;
    }

    if(sumOfCases == 3 || sumOfCases == 4)
    {
...
bool validateEmail(vector<string>& GM, string& email )
{
    int size = GM.size();
    for(int i = 0 ; i <= size - 1; i++  )
    {
...
void newuser()
{
    ...
    while(x == "User name")
    {
        ...
        getline(cin, username);
        if(validateUsername(NA, username))
        {
            checkValdition += 1;
        }
